package com.feather.loader.core;

import java.io.File;
import java.lang.instrument.Instrumentation;
import java.util.jar.JarFile;
import java.util.jar.JarEntry;
import java.util.Scanner;

public class ModLoader {

    public static void loadMods(Instrumentation inst) {
        String workingDir = System.getProperty("user.dir");
        File modsFolder = new File(workingDir, "feather_mods");

        if (!modsFolder.exists()) {
            modsFolder.mkdirs();
        }

        File[] files = modsFolder.listFiles((dir, name) -> name.endsWith(".jar"));
        if (files == null) return;

        for (File file : files) {
            try {
                System.out.println("[Feather] Finding potential mod: " + file.getName());
                loadModFile(file, inst);
            } catch (Exception e) {
                System.err.println("[Feather] Failed to load mod: " + file.getName());
            }
        }
    }

    private static void loadModFile(File file, Instrumentation inst) throws Exception {
        JarFile jarFile = new JarFile(file);
        inst.appendToSystemClassLoaderSearch(jarFile);
        
        JarEntry entry = jarFile.getJarEntry("feather.mod.json");
        if (entry != null) {
            Scanner s = new Scanner(jarFile.getInputStream(entry)).useDelimiter("\\A");
            String json = s.hasNext() ? s.next() : "";
            String mainClass = parseJsonField(json, "main");
            String modName = parseJsonField(json, "name");

            if (mainClass != null) {
                Class<?> clazz = Class.forName(mainClass);
                Object modInstance = clazz.getDeclaredConstructor().newInstance();
                clazz.getMethod("init").invoke(modInstance);
                System.out.println("  [Feather] Mod [" + modName + "] Initialized");
            }
        }
        jarFile.close();
    }

    private static String parseJsonField(String json, String field) {
        try {
            String key = "\"" + field + "\":";
            int start = json.indexOf(key) + key.length();
            int firstQuote = json.indexOf("\"", start) + 1;
            int lastQuote = json.indexOf("\"", firstQuote);
            return json.substring(firstQuote, lastQuote);
        } catch (Exception e) {
            return null;
        }
    }
}
